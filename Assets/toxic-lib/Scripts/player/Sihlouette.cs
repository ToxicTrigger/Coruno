...
public class Sihlouette : MonoBehaviour, IPunObservable
{
    ...
    void Update()
    {
        ...
        {
            m_PositionModel.onPlatform = this.transform.parent == null ? false : true;
        }
    ...
    void UpdateRotation()
    {
        ...
        transform.GetChild(0).localRotation = this.m_RotationControl.GetRotation(transform.GetChild(0).localRotation);
    }
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            stream.SendNext(m_PositionModel.onPlatform);
        }
        else
        {
            m_PositionModel.onPlatform = (bool)stream.ReceiveNext();
        }

        this.m_PositionControl.OnPhotonSerializeView(this.transform, stream, info, m_PositionModel.onPlatform);
        this.m_RotationControl.OnPhotonSerializeView(transform.GetChild(0).localRotation, stream, info);
        ...
        if (stream.IsReading == true)
        {
            ...
            if (m_firstTake)
            {
                ...
                {
                    this.transform.GetChild(0).localRotation = this.m_RotationControl.GetNetworkRotation();
                }
...
public class PhotonTransformViewPositionControl
{
    ...
    public void OnPhotonSerializeView(Transform T, PhotonStream stream, PhotonMessageInfo info, bool onPlatform)
    {
        ...
        if (stream.IsWriting == true)
        {
            if (onPlatform)
            {
                SerializeData(T.localPosition, stream, info, onPlatform);
            }
            else
            {
                SerializeData(T.position, stream, info, onPlatform);
            }
        }
        else
        {
    ...
    void SerializeData(Vector3 currentPosition, PhotonStream stream, PhotonMessageInfo info, bool onPlatform)
    {
    ...
    void DeserializeData(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        if (m_Model.onPlatform)
        {
            if (game_manager.instance.flags.flags["LookAt"])
            {
                if (info.photonView.gameObject.name.GetHashCode() == game_manager.instance.player_1_hash)
                {
                    readPosition.y += 1000;
                }
                else
